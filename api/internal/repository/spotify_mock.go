...
func (m *MockSpotify) RecommendPopular(ctx context.Context, target AudioFeatures, market string, minPopularity, limit int) ([]Track, error) {
	...
	jitter := func(x, amplitude float64) float64 { return clamp01(x + (r.Float64()*2-1)*amplitude) }
	jitterTempo := func(bpm, amp float64) float64 { return math.Max(60, math.Min(200, bpm+(r.Float64()*2-1)*amp)) }
